...
if(showQuantitySaleItems) {
  saleProducts = saleProducts.sort((a, b) => {
  ...
  return (
    ...
    // <Link to={`/products/${product.id}`} key={`${product.id}`} className={s.product}>
    <div className={`${s.wrapper} container`}>
      <div className={s.title_btn}>
        <h2 className={s.title}>Sale</h2>
        {
        showQuantitySaleItems &&  <div className={s.category_line_container}>
        <div className={s.categories_line}></div>
        <Link to="/sales"> <AllProductsBtn buttonText="All sales"/></Link> </div>
        } 
      ...
      <div className={s.category_container}>
        {saleProducts.map((product) => (
          <div className={s.category_wrapper} key={`${product.id}`}>
            ...
            <div className={s.content_wrapper}>
              <p className={s.category_title} title={`${product.title}`}>{`${product.title}`} </p>
